...
int a, b, fl, fr, sl, sr, sub, fr_vol, fl_vol, sr_vol, sl_vol,sub_vol, mas_vol, in;
...
void eeprom_update() {
  ...
  EEPROM.update(1, fl_vol);
  EEPROM.update(2, fr_vol);
  EEPROM.update(3, sl_vol);
  EEPROM.update(4, sr_vol);
  EEPROM.update(5, sub_vol);
}

void eeprom_read() {
  mas_vol = EEPROM.read(0);
  fl_vol = EEPROM.read(1);
  fr_vol = EEPROM.read(2);
  sl_vol = EEPROM.read(3);
  sr_vol = EEPROM.read(4);
  sub_vol = EEPROM.read(5);
}
